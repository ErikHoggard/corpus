...
export default function MidiInstrument() {
	...
	function Sample({ sample, midiAccess, selectedInput, setSamples }) {
		...
		const playNote = (note, velocity) => {
			...
			const originalPitch = 60; // replace with the MIDI note number of the sample's original pitch
		...
		const stopNote = (note) => {
			...
			if (noteIDsForNote.length > 0) {
				...
				if (playingNotes[noteID]) {
					// playingNotes[noteID].source.stop(endPoint);
